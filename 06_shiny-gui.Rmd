...
The GWSDAT user interface is a web-based graphical user interface (GUI) using the R Shiny framework (`www.shiny.rstudio.com`) which the user can interact with in many ways. The following sections will explain the interface in more detail.


## Data Input via Graphical User Interface {#DataInputviaGUI}

The functionality to input data directly via the GUI is a feature first introduced in GWSDAT version 3.0. The user is referred to Section \@ref(DataInput) for full details on the format of the input data. 
