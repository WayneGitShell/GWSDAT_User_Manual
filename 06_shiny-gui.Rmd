...
The GWSDAT user interface is a web-based graphical user interface (GUI) using the R Shiny framework (`https://shiny.posit.co/`) which the user can interact with in many ways. The following sections explain the interface in  detail.
...
The listbox *Select wells to be omitted from analysis* allows the user to select any combination of wells from the complete list of wells. The order of wells in the list box are presented such that the wells which have been estimated to have the least influence on the spatiotemporal solute concentration smoother are presented first. This order is established via a procedure fully documented [here](https://github.com/peterradv/Well-Influence-Analysis) and [here](https://eprints.gla.ac.uk/303347/1/303347.pdf). This approximation procedure has been adopted to greatly increase computational speed and efficiency. For this reason, the order of wells is to be interpreted as a guide only. Please note that this order is also dependent on the current selection of omitted wells and substance and will be updated once the model is updated or a different substance selected. For this reason, it is suggested to omit wells in an incremental one-by-one fashion.
...
## Well Time Series Plot {#SectionWellTrendPlot}
...
Since GWSDAT version 3.3, users can modify the plotting date range and select multiple monitoring wells and multiple substances. Simply add and remove selections from the *Substance* and *Select Monitoring Well* listboxes. This produces a matrix of individual time series plots where the monitoring wells are arranged in rows and the substances are arranged in columns. The slider at the right hand foot of the page controls the date range.  Note that the slider box at the foot of the page can be
moved to any convenient position by clicking and dragging with the mouse. Figure \@ref(fig:MultivarGWSDATTimeSeriesPlot) displays an example GWSDAT Matrix Time Series plot with a subset of the original date range. 
...
Please note that the date range selection only applies to the Time Series plot and does not influence the Trend and Threshold Indicator plot which is always calculated from the full data set - see Section @ref(TRendTHreshMat)
}
}
}
\end{center}
...
```{r, fig.cap = "GWSDAT Matrix Time Series Plot with date range selection.", label="MultivarGWSDATTimeSeriesPlot",out.width = '80%', echo = FALSE,fig.pos="!h"}
include_graphics("figures/time-series-plot_V3.3.png")
```

The *Solute Conc. Unit* radiogroup allows the units to be changed between one of 'ng/l', 'ug/l' or 'mg/l'. The *Display Threshold* checkbox allows the user to overlay threshold value used to colour code the Trend and Threshold Indicator plot - see Section \@ref(TRendTHreshMat). 
...
* **Aggregate by**: provides a drop-down menu which allows the temporal plotting resolution to be altered (Day, Month, Quarter, Semi-Annual, Year).  This functionality is identical to the 'Aggregate by' option detailed in Section \@ref(SectionSpatialPlot).

* **Substance**: listbox to add and remove different substances from the plot. 
...
The choice of which solutes and wells to include, together with the choice of whether to use a log-scale for the solute concentration values, is selected by the user from the controls on the left hand side of the plot. In addition, it is possible to select/deselect all wells. After edits are made to display, please click on 'Update Plot'.
...
This selection, located under the 'More' tab, allows the user to save and download the current analysis session to a '.rds' file format. This is particularly useful for large data sets which may take a long time to fit the underlying models. The current display settings, e.g. Colour Scheme, Plot Options, Current Solute, are saved. This '.rds' file can be forwarded to another user and viewed once again exactly as it was saved. In version 3.3 there is an option to save a session file in a backwards compatible format. This is to avoid issues related to version 3.3 multiple well/solute selection in the Well Time-Series plot not being support in older versions. 
