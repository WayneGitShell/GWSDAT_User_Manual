...
The *Well Redundancy Analysis* feature described in Section \@ref(WellRedundancy) allows the user to very conveniently drop a well or a combination of wells from the analysis and investigate the resultant impact. The order of monitoring wells to potentially omit is presented to the user in such a way that the monitoring wells which have been estimated to have the least influence on the spatiotemporal solute concentration smoother are presented first. The ordered list of monitoring wells is created by calculating the average Cook's distance value of the observations in each well. The Cook's distance @cook, devised by Ralph Dennis Cook in 1977, is commonly used to estimate the influence of a data point in least-squares regression analysis. It is a measure of the change in regression estimates if an observation is deleted. In a P-splines model, it can be expressed using standardised residuals, the effective degrees of freedom and leverages:


\begin{equation}
    C_i = \frac{1}{p} (r^s_i)^2 \frac{h_{ii}}{1-h_{ii}}.
\end{equation}

The effective degrees of freedom $p$ is the trace of the hat matrix $H$, see Section \ref{SSCS}, equation (\ref{eq:Hat}). The leverage of the $i$-th observation $h_{ii}$ is the diagonal element of the hat matrix $H$ in row $i$ and column $i$. The standardised residual of the $i$-th observation is represented by $r^s_i$. Standardised residuals are the differences between an observation and its corresponding fitted value divided by an estimate of their standard deviation and defined as follows:

\begin{equation}
    r^s_i = \frac{(y_i-\hat{y}_i)}{\sqrt{\frac{\sum_{i=1}^{n}{(y_i-\hat{y}_i)^2}}{n-p}}\sqrt{1-h_{ii}}},
\end{equation}

where $y_i$ is the $i$-th observation and $\hat{y}_i$ is its corresponding fitted value and $n$ is the number of observations.
...
The underlying research and development justification for selecting the Cook's distance over a range of other measures is fully described and documented [here](https://github.com/peterradv/Well-Influence-Analysis).  


<!-- Cook's distance (CD), devised by Ralph Dennis Cook (1977), is an influence statistic used in regression analysis that measures the change in estimates if an observation is deleted. It can be used to identify observations that need to be checked for validity due to their disproportionate influence, or to indicate regions of the design space where more data points would be beneficial. In the context of GWSDAT's Well Redundancy Analysis, CD is used to order monitoring wells by their influence on the solute concentration estimates. The CD value of an observation can be expressed using information from the spatiotemporal P-Splines model fitted by GWSDAT (section 7.1), namely the standardised residuals, the effective degrees of freedom of the model and the leverages, as shown in the following equation: -->

<!-- \begin{equation} -->
<!--     C_i = \frac{1}{p} (r^s_i)^2 \frac{h_{ii}}{1-h_{ii}}. -->
<!-- \end{equation} -->

<!-- The effective degrees of freedom $p$ is the trace of the hat matrix $H$ (see 7.1, equation x). The leverage of the $i$-th observation $h_{ii}$ is the diagonal element of the hat matrix $H$ in row $i$ and column $i$. The standardised residual of the $i$-th observation is represented by $r^s_i$. Standardised residuals are the differences between an observation and its corresponding fitted value divided by an estimate of their standard deviation. They can be expressed using the following equation: -->

<!-- \begin{equation} -->
<!--     r^s_i = \frac{(y_i-\hat{y}_i)}{\sqrt{\frac{\sum_{i=1}^{n}{(y_i-\hat{y}_i)^2}}{n-p}}\sqrt{1-h_{ii}}}, -->
<!-- \end{equation} -->

<!-- where $y_i$ is the $i$-th observation and $\hat{y}_i$ is its corresponding fitted value and $n$ is the number of observations. The ordered list of monitoring wells is created by calculating the median CD value of the observations in each well. The median is calculated using the following formulae: -->

<!-- \begin{equation} -->
<!--     Med(L_j) = \begin{cases} -->
<!--     L_j\frac{n+1}{2}, & \text{if n is odd}.\\ -->
<!--     \frac{L_j[\frac{n}{2}]+L_j[\frac{n}{2}+1]}{2}, & \text{if n is even}, -->
<!--     \end{cases} -->
<!-- \end{equation} -->

<!-- where $L_j$ is the ordered list of CD values of observations from monitoring well $j$, where $j=1,2,...,m$ and $m$ is the number of monitoring wells. The influence of a monitoring well on regression estimates is then proportional to the median CD value of its observations: -->

<!-- \begin{equation} -->
<!--     I_j \propto Med(L_j). -->
<!-- \end{equation} -->

<!-- Thus, the monitoring wells are arranged in increasing numerical order based on their estimated influence, $I_j$. -->
