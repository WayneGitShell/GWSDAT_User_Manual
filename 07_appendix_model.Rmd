...
Plume mass is calculated from the scaled product of plume area and average concentration. The scaling factor encompasses the user specified value of ground porosity (see [`Plume Diagnostics`](#PlumeDiagnostics)) and appropriate scaling values for mapping together the volumetric concentration units (e.g. ug/l) with the length scale (see \textbf{CoordUnits} in [`Well Coordinates Table`](#WellCoordsTabExplan)) of the Well coordinates (e.g. metres or feet). 
...
The plume center of mass $(x,y)$ is defined as the mean location of the concentration distribution within the plume boundary region $D$. 
The x-coordinate of the plume Centre of Mass is evaluated by numerical calculation of 
...
## Well Trend Plot Smoother {#smregressCalc}

The well trend plot smoother is fitted using a nonparametric method called local linear regression. 
