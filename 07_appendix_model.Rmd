...
The Well Redundancy Analysis feature described in Section \@ref(WellRedundancy) allows the user to very conveniently drop a well or a combination of wells from the analysis and investigate the resultant impact. The order of wells to potentially omit is presented to the user in such a way that the wells which have been estimated to have the least influence on the spatiotemporal solute concentration smoother are presented first. This order is established using the average Cook's distance from each data point for a particular well. The Cook's distance @cook, devised by Ralph Dennis Cook in 1977, is commonly used to estimate the influence of a data point in least-squares regression analysis. It is a measure of the change in regression estimates if an observation is deleted. In a P-splines model, it can be expressed using standardised residuals, the effective degrees of freedom and leverages:
